...
def start(target, counter, delay, ch, cc):
    ...
    while success < (int(bombs)):
        ...
        try:
            ...
            if cc == "91":
                print('Sorry All APIs Have Expired Please Update HKaboom')
                input('Press Enter To Exit...')
                exit()
            else:
                if success > 0:
                    print(
                        '\n\n\tThe Bombing Limit For this Country Has Been Reached...')
...
while True:
    pn = ""
    cc = input("\tEnter the Country Code (Without +) : ")
    if '+' in cc:
        tc = list(cc)
        tc.remove('+')
        cc = ''.join(tc)
        cc = cc.strip()
    pn = input("\tEnter the Target's Number: +" + cc + " ")
    pn = remsp(pn)
...
try:
    ...
    type = 0
if type == 1:
    nm = int(input("Number of Calls : "))
    if nm > 1500000:
     
        nm = 1500000
    dl = float(input("Enter Delay time: "))
elif type == 0:
    if cc == "91":
        nm = int(input("Number of Messages To Send: "))
        dl = float(
            input("Enter Delay time: "))
    else:
        nm = int(input("Enter Number of Messages To Send: "))
        dl = float(
            input("Enter Delay time (10 if not from india)  : "))
maxlim = 0
if cc == "91":
    maxlim = 5000000
else:
